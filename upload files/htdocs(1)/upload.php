...
&& ($_FILES["pict"]["size"] < 100000))
  ...
  if ($_FILES["pict"]["error"] > 0)
    ...
    if (file_exists("upload/" . $_FILES["pict"]["name"]))
      {
      echo $_FILES["pict"]["name"] . " already exists. ";
      }
    else
      {
      move_uploaded_file($_FILES["pict"]["tmp_name"],
      "upload/" . $_FILES["pict"]["name"]);
      echo "Stored in: " . "upload/" . $_FILES["pict"]["name"];
      }
    }
  }
else
  ...
  $picture = 'upload/'.$_FILES['pict']['name'];
/*
...
if($i_name!=NULL)
{
$result1=mysql_query("insert into item(i_id,i_name,color,deadline,description,location,pict) values(NULL,'$i_name','$color','$deadline','$description','$location','$picture')");
}
